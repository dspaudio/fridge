...
현재 0.3 릴리즈 빌드는 Apple Developer 인증서로 서명되지 않은 개발용 ad-hoc 서명 앱입니다. 이 때문에 macOS에서 다음 현상이 발생할 수 있습니다.
...
- `fridgeAwareness`: Fridge hook이 설치되어 있고 agent child process를 `SIGSTOP`/`SIGCONT`로 freeze/resume할 수 있다는 런타임 계약
...
- `hookContext`: hook stdin/argument payload에서 추출한 `lastTool`, `cwd`, `pendingPromptSummary`
- `resumeProbe`: frozen child/TTY가 아직 살아 있는지 확인한 probe 결과
- `thawResult`: `resumed`, `child_gone`, `tty_lost`, `unsafe_to_resume` 중 하나
- `agentInstruction`: agent가 다음 행동을 정할 때 우선 읽어야 하는 지시
- `message`: agent가 바로 읽을 수 있는 한 문장 요약

기본 설치는 `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `Stop`, `SessionEnd` 이벤트에 bridge를 걸어, agent가 어떤 hook 표면에서든 현재 Fridge 상태를 다시 인지할 수 있게 한다.
